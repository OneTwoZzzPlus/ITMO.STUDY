...
class DirectMultipleMeasurement(MultipleMeasurement):
    ...
    def __init__(self, 
            measured_values:list[int|float|Measurement], 
            delta_instrumental:int|float=0, 
            dim:int|None=None,
            name:str|None=None, char:str='', unit:str=''):
        self._delta_instrumental_ = delta_instrumental
        if len(measured_values) == 0:
            raise MeasException("Пустой список!")
        if isinstance(measured_values[0], Measurement):
            ...
            self._measurments = [
                Measurement(x, delta_instrumental, direct=True) 
                for x in measured_values
            ]
        if dim is not None:
    ...
    def _calc(self):
        ...
        self._delta_ = sqrt(self._delta_random_**2 
                            + (4/9)*self._delta_instrumental_**2)
        if not isclose(self._value_, 0):
    ...
    def info(self):
        return (
            super().info() + 
            f"\nsigma_ = {self._sigma_}\n" +
            f"delta_instrumental = {self._delta_instrumental_}" +
